...
local function getcommit(sub: number): ()
	sub = sub or 7;
	local commitinfo: table = httpService:JSONDecode(httpasync('https://api.github.com/repos/joemama696969693355/ISkidYea/commits'))[1];
...
local function downloadFile(path, func, bypass)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/joemama696969693355/ISkidYea/'..readfile('newcatvape/profiles/commit.txt')..'/'..select(1, path:gsub('newcatvape/', '')), true)
		end)
